...
| `bbb_ip` | IP for this BigBlueButton instance| |Define it, if you the server can't resolve bbb_hostname locally.|
...
| `bbb_ssl_certificate` | SSL certificate | | To protect `https://{{ bbb_hostname }}`. Only if `bbb_letsencrypt_enable` == false |
| `bbb_ssl_private_key` | SSL private key | | To protect `https://{{ bbb_hostname }}`. Only if `bbb_letsencrypt_enable` == false |
